...
const TASKS = [
  ...
  { id: "level-2-task-12", level: 2, number: 12, title: "Technology for children", opening: "My twelve-year-old wants a smartphone because all her friends have one, but I’m worried she isn’t ready for it.", facts: ["You are her parent and share major parenting decisions with your partner; your partner's view is not yet settled.", "She currently uses a family tablet at home.", "She travels to school alone, so contact would be useful.", "A previous online argument upset her badly.", "She says she feels excluded socially.", "The cost is manageable."], options: ["basic phone", "smartphone with controls", "trial period", "wait"] },
  ...
  { id: "level-2-task-14", level: 2, number: 14, title: "Volunteering pressure", opening: "I agreed to help a local charity, but they now expect far more time than I can realistically give.", facts: ["Your volunteer role is administrative support and organising appointments for vulnerable families.", "You agreed to four hours per month.", "They now contact you several times a week.", "The work supports vulnerable families and feels meaningful.", "The organiser is kind but disorganised.", "Your paid work and health are suffering."], options: ["set boundaries", "reduce tasks", "take a break", "leave"] },
  { id: "level-2-task-15", level: 2, number: 15, title: "Noisy neighbours", opening: "My new neighbours regularly make noise late at night, but I’m reluctant to complain because I want us to have a good relationship.", facts: ["You rent a flat, and the neighbours moved into the flat next door three months ago.", "Loud music happens two or three nights a week.", "You start work early.", "You have spoken casually once; they apologised, but it continued.", "They seem friendly during the day.", "Other neighbours are also affected."], options: ["direct conversation", "written message", "shared solution", "landlord or community complaint"] },
...
async function createFeedback(request, env) {
  ...
  const comparison = previousTranscript?.trim() ? `\n\nThis is a repeat attempt of the same task. After RESULTADO GENERAL, add a concise section titled COMPARACIÓN CON EL INTENTO ANTERIOR. Compare only evidence visible in the two transcripts, identify genuine improvement, what still needs work, and whether earlier targets were achieved. Do not claim improvement when the evidence is unclear.\n\nPrevious attempt transcript:\n${previousTranscript}\n\nPrevious feedback (use only to recover earlier targets):\n${previousFeedback || "Not available."}` : "";
...
- Infer the student's communicative intent from their actual words and the conversation: question, advice, empathy, agreement, rapport, personal experience or development.
...
const TASKS=${JSON.stringify(CLIENT_TASKS)},TOTAL=240;let selectedTask=null,status='ready',remaining=TOTAL,turns=[],pc=null,dc=null,timerId=null,openingPending=false,micTracks=[],weakTurnCount=0,supportCount=0,audioContext=null,turnInFlight=false,previousAttempt=null,attemptNumber=1;
...
function showTasks(level=selectedTask?.level||1){stopConnection();selectedTask=null;previousAttempt=null;attemptNumber=1;el('taskLevelHeading').textContent='LEVEL '+level+' · '+(level===1?'CORE PRACTICE':'CHALLENGE PRACTICE');renderTasks(level);el('levelChooser').classList.add('hidden');el('practiceCard').classList.add('hidden');el('transcriptCard').classList.add('hidden');el('feedbackCard').classList.add('hidden');el('taskChooser').classList.remove('hidden')}
function selectTask(task){selectedTask=task;previousAttempt=null;attemptNumber=1;el('taskTitle').textContent='Task '+task.number+' · '+task.title;el('levelLabel').textContent='LEVEL '+task.level;el('taskChooser').classList.add('hidden');el('practiceCard').classList.remove('hidden');remaining=TOTAL;updateTimer();setStatus('ready')}
...
function classifyStudentTurn(text){const raw=(text||'').trim();if(!raw)return 'weak';const compact=raw.toLowerCase().replace(/[^a-z0-9 ]/g,' ').replace(/\s+/g,' ').trim();const fillers=new Set(['ah','oh','uh','um','erm','er','mm','mhm','hmm','okay','ok','well']);if(!compact||compact.split(' ').every(word=>fillers.has(word)))return 'weak';return 'auto'}
