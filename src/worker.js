...
- Treat “I’m still not sure what to do” as minimal examiner prompting, not as a supplied conversational route.
...
- Never ask a question, teach, praise, correct, rescue or provide a conversational route.
...
function cleanControlledReply(text, mode) {
  const fallback = mode === "empathy" ? "Thank you, it is difficult." : mode === "advice" ? "I’m not sure that would be best." : mode === "comment" ? "Yes, I understand." : "I’m not sure.";
...
function classifyStudentTurn(text){const raw=(text||'').trim();if(!raw)return 'weak';const words=raw.toLowerCase().replace(/[^a-z0-9' ]/g,' ').split(' ').filter(Boolean);const compact=words.join(' ');const advice=['maybe','perhaps','i think','you could','you should','you might','you need','you have to','if i were','have you thought','why don t you','what about if'];if(advice.some(phrase=>compact.includes(phrase)))return 'advice';if(raw.includes('?'))return 'question';const questionStarts=new Set(['who','what','when','where','why','how','do','does','did','is','are','was','were','can','could','would','will','have','has','had']);if(questionStarts.has(words[0]))return 'question';const empathy=['that must be','that sounds difficult','that sounds hard','that sounds frustrating','very difficult','difficult situation','must be difficult','must be frustrating','very frustrated','understand how you feel','understand your situation','that s understandable','that is understandable','similar situation','similar problem','same problem','happened to me'];if(empathy.some(phrase=>compact.includes(phrase)))return 'empathy';const briefComments=['sounds good','good idea','that could work','might work'];if(briefComments.some(phrase=>compact.includes(phrase)))return 'comment';if(words.length>=8)return 'comment';return 'weak'}
