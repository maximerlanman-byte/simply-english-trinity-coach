...
function renderTopics(){el('topicGrid').textContent='';TOPICS.forEach(item=>{const b=document.createElement('button');b.className='topic';const title=document.createElement('strong');title.textContent=item.name;const detail=document.createElement('span');detail.textContent=item.description;b.append(title,detail);b.onclick=()=>chooseTopic(item,true);el('topicGrid').appendChild(b)});const surprise=document.createElement('button');surprise.className='topic surprise';const title=document.createElement('strong');title.textContent='Surprise me';const detail=document.createElement('span');detail.textContent='Choose a random topic and a fresh question for me.';surprise.append(title,detail);surprise.onclick=()=>chooseTopic(TOPICS[Math.floor(Math.random()*TOPICS.length)],true);el('topicGrid').appendChild(surprise)}
function chooseTopic(item,autoStart=false){topic=item;current=null;previousAttempt=null;el('topicLabel').textContent=item.name.toUpperCase();el('topicsCard').classList.add('hidden');el('practiceCard').classList.remove('hidden');el('transcriptCard').classList.add('hidden');el('feedbackCard').classList.add('hidden');el('start').classList.remove('hidden');setStatus('ready','Ready for a surprise question');if(autoStart)startPractice(false)}
