...
function onEvent(event){if(event.type==='conversation.item.input_audio_transcription.completed'){if(openingPending||turnInFlight||status!=='active')return;const text=event.transcript||'';addTurn('Student',text);if(text.trim()){const kind=classifyStudentTurn(text);if(kind==='question'||kind==='advice'||kind==='comment'){weakTurnCount=0;requestExaminerResponse(kind,text)}else if(kind==='empathy'){weakTurnCount=0;requestExaminerResponse('empathy',text)}else{weakTurnCount++;if(weakTurnCount>=2&&supportCount<3){weakTurnCount=0;supportCount++;requestExaminerResponse('nudge',text,false,supportCount)}}}}if(event.type==='error'){console.error(event);showError('The voice connection reported an error. Please try again.')}}
